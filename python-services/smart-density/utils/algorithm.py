...
class Controller:
    def __init__(self, dt=1, para1=5, para2=0.0, para3=1, u_min=float('-inf'), u_max=float('inf')):
        ...
        self.u_min = u_min
        self.u_max = u_max
        self.prev_output = 0  # 记录上一次的控制量输出
        # print("控制器初始化")
    ...
    def update_FL_AntiWindup(self, setpoint, process_variable, para1, para2, para3):
        ...
        print("---分流：参数1:%.2f---参数2:%.2f---参数3:%.2f---" % (para1, para2, para3))
    ...
    def update_BS_AntiWindup(self, setpoint, process_variable, para1, para2, para3):
        ...
        print("---补水：参数1:%.2f---参数2:%.2f---参数3:%.2f---" % (para1, para2, para3))
        return output
    ...
    def update_Jiayao_AntiWindup(self, setpoint, process_variable, para1, para2, para3):
        ...
        # 计算未受限的控制量输出
        output_unconstrained = para1 * error + para2 * self.integral + para3 * derivative
        print("output_unconstrained = %.4f" % (output_unconstrained))

        if (output_unconstrained >= self.u_max and error > 0):
            print("self.integral 调整前= %.4f" % (self.integral))
            integral_adjustment = (output_unconstrained - self.u_max) / (para1 / 3)
            self.integral -= integral_adjustment
            print("self.integral 调整后= %.4f" % (self.integral))

        else:
            self.integral += error * self.dt
            print("self.integral = %.4f" % (self.integral))

        output_adjusted = para1 * error + para2 * self.integral + para3 * derivative
        print("output_adjusted = %.4f" % (output_adjusted))
        ...
        print("参数1:%.2f---参数2:%.2f---参数3:%.2f  " % (para1, para2, para3))
        return output
...
class MPCController:
    def __init__(self, dt=1, N=10, Q=50, R=1, u_min=int('0'), u_max=int('1')):
        ...
        self.N = N  # 预测时间步长
        #模型方程
        self.A = np.array([[0, 0, 0], [1, 0, 0], [0, 0.004, 0.985]])  # 0.985
        self.B = np.array([[1.0], [0], [0]])
        self.C = np.array([[0, 0, 1]])
        self.U_prev = np.array([[1, 0, 0]])

        self.x = cp.Variable((self.A.shape[0], N+1))      # X=A行 N+1列个 保存预测周期内的值
        self.u = cp.Variable((self.B.shape[1], N))        # U=B的列数
        self.u_min = u_min
        self.u_max = u_max        #控制器界限

        # self.Q = np.zeros([self.A.shape[0], self.A.shape[0]])  # 扩维后的Q矩阵
        # self.Q[self.A.shape[0] - 1, self.A.shape[0] - 1] = Q
    ...
    def updata(self, yr, measured, u_prev):
        ...
        constraints = [self.x[:, 0] == x_curr[:, 0]]  # 初始约束

        for k in range(self.N):
            if k >= 1:
                cost += cp.quad_form(self.C @ self.x[:, k] - yr, self.Q) + cp.quad_form(self.u[:, k] - self.u[:, k - 1], self.R)  # 代价函数
            else:
                cost += cp.quad_form(self.C @ self.x[:, k] - yr, self.Q) + cp.quad_form(self.u[:, k] - self.U_prev @ self.x[:, k], self.R)# 代价函数
            constraints += [self.x[:, k + 1] == self.A @ self.x[:, k] + self.B @ self.u[:, k]]  # 状态更新约束
            constraints += [self.u[:, k] >= self.u_min, self.u[:, k] <= self.u_max]  # 控制信号约束
        # 定义优化问题
